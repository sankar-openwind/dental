angular.module("dentalApp").controller('DiagnosisController', ['$scope', '$http', '$templateCache', '$location', '$routeParams', '$rootScope', function ($scope, $http, $templateCache, $location, $routeParams, $rootScope) {
    ...
    $scope.getDiagnosisInformation = function () {
        ...
        var regurl = $rootScope.restURL + '/getCurrentDiagnosis/' + patientid;
        $http.get(regurl).success(function (data) {
            console.log("got the patient details " + patientid + data);
            $scope.diagnosisInformation = addSymbols(data.diagnosisData);
    ...
    $scope.newDiagnosis = function () {
        var patientid = $routeParams.patientid;
        $scope.diagnosisInformation = {};
        var regurl = $rootScope.restURL + '/newDiagnosis/' + patientid;
        $http.get(regurl).success(function (data) {
            console.log("got the patient details " + patientid + data);
            $scope.diagnosisInformation = {};
            $scope.patient.patientid = patientid;
            ...
            var inserturl = $rootScope.restURL + '/newDiagnosis/';

            $scope.master = {};
            $scope.master.patientId = patientId;
            var jdata = 'mydata=' + removeSymbolsAndStringify($scope.master);
            $http({
                method: method,
                url: inserturl,
                data: jdata,
    ...
    $scope.saveDiagonisSummary = function (diagnosisInformation) {
        ...
        var method = 'POST';
        var inserturl = $rootScope.restURL + '/saveDiagnosisMaster';
        ...
        var jdata = 'mydata=' + removeSymbolsAndStringify($scope.master);
        console.log("Diagnosis data sent to the server" + jdata);
        $http({
            method: method,
            url: inserturl,
            data: jdata,
            ...
            $scope.$emit('success', 'Successfully saved diagnosis information');
        }).error(function (response) {
            console.log("error");
            $scope.codeStatus = response || "Request failed";
            console.log($scope.codeStatus);
    ...
    $scope.blq = function () {
     ...
     $scope.fullMouth = function () {
        if (!$scope.startedfullMouth) {
            $scope.startedfullMouth = true;
            insertData($scope.diseaseType, $scope.selectedDiseaseSubType, "Full Mouth");
        } else {
            removeData($scope.diseaseType, $scope.selectedDiseaseSubType, "Full Mouth");
            $scope.startedfullMouth = false;
        }
    };
    $scope.brq = function () {
        if (!$scope.startedbrq) {
    ...
    function insertData(diseaseType, selectedDiseaseSubType, teeth) {
        ...
        var diseaseType = $scope.diseaseType;
        var selectedDiseaseSubType = $scope.selectedDiseaseSubType;
        ...
        if (typeof $scope.diagnosisInformation[diseaseType] == "undefined") {
            // alert("Inserting the new data");
            $scope.diagnosisInformation[diseaseType] = {};
            $scope.diagnosisInformation[diseaseType][selectedDiseaseSubType] = [];
            $scope.diagnosisInformation[diseaseType][selectedDiseaseSubType].push(teeth);
        } else {
            if (typeof $scope.diagnosisInformation[diseaseType][selectedDiseaseSubType] == "undefined") {
                $scope.diagnosisInformation[diseaseType][selectedDiseaseSubType] = [];
                $scope.diagnosisInformation[diseaseType][selectedDiseaseSubType].push(teeth);
    ...
    function removeData(diseaseType, selectedDiseaseSubType, teeth) {
        var diseaseType = $scope.diseaseType.replace(/ /g, ' ');
        var selectedDiseaseSubType = $scope.selectedDiseaseSubType.replace(/ /g, ' ');
        var index = $scope.diagnosisInformation[diseaseType][selectedDiseaseSubType].indexOf(teeth);
        if (index > -1) {
            $scope.diagnosisInformation[diseaseType][selectedDiseaseSubType].splice(index, 1);
        }
    };

    //  REMOVES + AND - SYMBOLS TO THE PASSED JSON DATA
    var removeSymbolsAndStringify = function (data) {
        return JSON.stringify(data).replace(/\+/g, "pos").replace(/-/g, "neg");
    }
    ...
    var addSymbols = function (data) {
        ...
        console.log(data);
        console.log("After");
        console.log(JSON.parse(JSON.stringify(data).replace(/pos/g, "\+").replace(/neg/g, "-")));
